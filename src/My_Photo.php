...
<form method="post" action="delete.php">
    <div class="detail">
        ...
        $user=$_SESSION["user"];
        $sql = "SELECT * FROM myphoto WHERE (userName='$user')";
        ...
        if($value==0){
            ...
            for ($i=0;$i<$m;$i++){
                $j = $i + $l - 1;
                $sql = "SELECT * FROM myphoto WHERE (userName='$user') limit $j,1";
                $query = mysqli_query($conn,$sql);
 ...
 <figure><a href=\"Details.php?id=myPhoto&src=$src&title=$t&des=$des&path=$path&content=$sub&country=$country&city=$city\"><img src=".$src." style=\"float: left\" width=\"96\" height=\"96\" alt=\"photo\" ></a></figure>
 <figcaption style='margin-bottom: 60px'><h3>$t</h3>
<p style='text-align: left'>$des</p>
<button><a href='delete.php?path=$path&user=$user'>Delete</a></button>
<button><a href='Upload.php?id=my_photo&path=$path&country=$country&city=$city&des=$des&content=$sub&title=$t'>Modify</a></button>
</figcaption>
</div>";
            }
    ...
    <div style="margin-left: 45%; background-color: white;margin-top: 800px">
        <?php
        error_reporting(0);
        if($all==1){
            if(!isset($_GET['page'])){
